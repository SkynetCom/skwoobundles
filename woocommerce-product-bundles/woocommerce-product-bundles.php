...
class WC_Bundles {
	...
	public function on_activation() {
		add_option( 'woocommerce_pb_plugin_activated', true );
	...
	/**
	 * Handle plugin after activation process.
	 *
	 * Because the plugin is not fully initialized on activation, this is useful for code that depends on the plugin being loaded.
	 *
	 * @since  8.5.6
	 *
	 * @return void
	...
	public function after_activation() {
		if ( ! is_admin() || ! get_option( 'woocommerce_pb_plugin_activated' ) ) {
			return;
		}

		// Delete the activation option to make sure the following code runs only once after activation.
		delete_option( 'woocommerce_pb_plugin_activated' );

		$this->maybe_define_constant( 'WC_PB_PLUGIN_ACTIVATED', true );
	}
	...
	public function on_deactivation() {
		...
		WC_PB_DB_Sync::maybe_cleanup_as_cronjob();
	}
}
